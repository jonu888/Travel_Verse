...
const SearchInput = ({ value, onChange, onFocus, onBlur, placeholder, icon }) => (
...
const MapDistance = ({ onDistanceCalculated, onClose }) => {
  ...
  const [filteredStartPlaces, setFilteredStartPlaces] = useState([]);
  const [filteredEndPlaces, setFilteredEndPlaces] = useState([]);
  ...
  const handleWaypointChange = React.useCallback((points, totalDistance) => {
  ...
  const handleMarkerDrag = React.useCallback((isStart) => (e) => {
    setIsCalculating(true);
    const marker = e.target;
    const position = marker.getLatLng();
    const newPoint = [position.lat, position.lng];
...
MapDistance.propTypes = {
  onDistanceCalculated: PropTypes.func.isRequired,
